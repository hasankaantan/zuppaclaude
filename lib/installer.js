...
class Installer {
  ...
  async checkDependencies() {
    ...
    // Check Node.js version (Claude Code v2.x requires 18+)
    const nodeVersion = process.version;
    const major = parseInt(nodeVersion.slice(1).split('.')[0]);
    if (major >= 18) {
      this.logger.success(`Node.js: ${nodeVersion}`);
    } else {
      this.logger.error(`Node.js: ${nodeVersion} (requires 18+)`);
      allOk = false;
    }
    ...
    if (this.platform.commandExists('claude')) {
      ...
      if (this.platform.isMac) {
        this.logger.info('Install: brew install --cask claude-code');
        this.logger.info('    or:  curl -fsSL https://claude.ai/install.sh | bash');
      } else if (this.platform.isWindows) {
        this.logger.info('Install: winget install Anthropic.ClaudeCode');
        this.logger.info('    or:  irm https://claude.ai/install.ps1 | iex');
      } else {
        this.logger.info('Install: curl -fsSL https://claude.ai/install.sh | bash');
      }
      this.logger.info('    or:  npm install -g @anthropic-ai/claude-code');
      allOk = false;
    }
